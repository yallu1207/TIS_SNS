...
var idCheck=function(){
	...
	if( !idReg.test( $("input[name=uid]").val() ) ) {
         alert("아이디는 소문자로 시작하는 6~20자 영어소문자 또는 숫자이어야 합니다.( 특수문자 사용불가 )");
         return;
    }
	
	$.ajax({
		type:'GET',
		url:'id_check?userid='+uid,
		dataType:'json',
		cache:false,
		...
		<div class="row" style="margin-top: 30px">
			<!-- <form name="idf" id="idf" action="id_check" method="post"> -->
				...
				<div class="col-xs-10 col-offset-2">
					...
					<button class="btn btn-primary" style="display:none" id="useid_change" onclick="change()">다른 아이디 찾기</button>
				</div>
		</div>
	</div>
...
<script>
	function set(){
		//alert(uid);
		opener.document.mf.userid.value=$('#uid').val();
